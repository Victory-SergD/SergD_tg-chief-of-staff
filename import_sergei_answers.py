#!/usr/bin/env python3
"""
Импорт ответов САМОГО Сергея из заполненного critical_for_sergei.md.

Идемпотентный — если файл пустой / нет новых ответов / нет файла → молча выходит.
Может вызываться из daily.sh без побочных эффектов.

→ disputed_questions.answer + answered=1
→ user_notes (через Opus, как АВТОРИТЕТНЫЙ факт от Сергея — приоритетнее всего)

Запуск:
  python3 import_sergei_answers.py [--file FILE] [--dry-run]
"""
import argparse
import json
import os
...
Формат: «[2026-05-08 от Сергея] факт1. факт2. факт3.»
...
def main():
    ...
    p.add_argument("--file", default="critical_for_sergei.md")
    p.add_argument("--dry-run", action="store_true")
    args = p.parse_args()

    path = BASE / args.file
    if not path.exists():
        print(f"⏭ Файл не найден: {path.name} — нечего импортировать")
        return

    answers = parse_filled_md(path)
    print(f"Найдено заполненных ответов: {len(answers)}")

    if not answers:
        print("⏭ В файле нет ответов — выходим без действий")
        return
